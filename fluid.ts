...
function generateClass(variant: string, prefix: keyof typeof utilityMap, start: string | number, end: string | number) {
  const cssProps = utilityMap[prefix];
  if (!cssProps) return `/* Unsupported utility: ${prefix} */`;

  const className = `.\\~${prefix}-${start.toString().replace("/", "\\/")}/${end}`.replace(
    "/",
    "\\/",
  );
  ...
  const value =
    prefix === "text"
      ? `clamp(${start}rem, 1vw + ${Number(start) / 2}rem, ${end}rem)`
      : clampSpacing(start, end);
...
function scanClassesFromFiles(root: string) {
  ...
  return (Array.from(found) as string[]).map((entry: string) => entry.split("|"));
}

function main() {
  const matches = scanClassesFromFiles(".") as [string, keyof typeof utilityMap, string | number, string | number][];
